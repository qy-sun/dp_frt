# ==============================================================================
# Figure 3: MSE of the naive and Bayesian denoising estimators
# ==============================================================================
...
sim_once <- function(n11, n10, n01, n00, epsilon,
  ...
  out <- tryCatch({
    ...
    p_naive  <- frt_p(a = t11, b = t01, n1 = n1, n0 = n0)
    ...
    c(p_true = p_true, p_naive = NA_real_, p_mean = NA_real_,
      p_median = NA_real_, p_map = NA_real_)
  })
  
  out <- as.numeric(out)
  names(out) <- c("p_true","p_naive","p_mean","p_median","p_map")
  out
}
...
sim_mse <- function(n11, n10, n01, n00, epsilon, S = 2000,
  ...
  res <- vapply(
    ...
    FUN.VALUE = c(p_true=0, p_naive=0, p_mean=0, p_median=0, p_map=0)
  )
  p_true <- res["p_true", ]
  mse <- function(est) {
...
plot_df_all <- mse_df_all |>
  ...
  mutate(method = factor(method,
                         levels = c("MSE_naive","MSE_mean","MSE_median","MSE_map"),
                         labels = c("Naive","Posterior Mean","Posterior Median","Posterior MAP")),
         sample_size = factor(sample_size, levels = c("small","medium","large"),
                              labels = c("n=50","n=100","n=500")),
...
fig3_grid <- ggplot(plot_df_all, aes(x = epsilon, y = MSE, color = method, group = method)) +
...
# ggsave("Fig3.png", fig3_grid, width = 10, height = 6, dpi = 1000)
# save(mse_df_all, file = "Fig3.Rdata")