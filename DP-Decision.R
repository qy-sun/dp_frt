...
.dpfrt_calibrate_freq_data_adaptive <- function(
  ...
  alpha1 <- alpha_freq - eta
  if (alpha1 <= 0) 
    stop("alpha_freq must be larger than eta in data-adaptive calibration.")
  ...
  for (idx in seq_along(K_vals)){
    ...
    i_star <- which(freq_tab$cum_prob >= (1 - eta))[1]
  ...
  if (length(K_conf) == 0L){
    message("Data-adaptive calibration: C_{1-eta}(tilde n) 
            is empty based on simulations; fallback to all K.")
    K_conf <- K_vals
  }
...
dp_frt_decision <- function(
  ...
  return(list(
    ...
    alpha       = alpha,
    alpha_freq  = alpha_freq,
    eta         = if (calib_res$calib_type == "data_adaptive") calib_res$eta else NULL,
    meta        = list(
      n1      = n1,
      n0      = n0,
      n       = n,
